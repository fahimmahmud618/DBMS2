...
public class SkinDitector {
    ...
    void calculateProbability() throws IOException
    {
        ...
        double temp=0;
        FileOutputStream outputStream = new FileOutputStream("prob_last.txt");
        ...
        for(int x=0;x<256;x++)
        {
            for(int y=0;y<256;y++)
            {
                for(int z=0;z<256;z++)
                {
                    skinProbability[x][y][z] = (double) skinColorRGB[x][y][z]/skinSum;
                    nonSkinProbability[x][y][z] = (double)nonSkinColorRGB[x][y][z]/noSkinSum;

                    if(skinProbability[x][y][z]==0.0) {
                        temp = 0;
                        a++;
                    }
                    else {
        ...
        System.out.println("skin "+a+" non: "+b);
        dataOutputStr.flush();
    }
}
